...
from api.schemas import StartGameRequest, JoinGameRequest, GameSessionResponse
...
async def create_game_route(game_request: StartGameRequest) -> GameSessionResponse:
    session = create_game(game_request)
    return GameSessionResponse.from_game_session(session)


@router.put("/join-game")
async def join_game_route(game_request: JoinGameRequest) -> GameSessionResponse:
    session = join_game(game_request)
    return GameSessionResponse.from_game_session(session)
